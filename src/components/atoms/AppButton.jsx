export function AppButton({ children, loading, variant = 'primary', fullWidth, style, ...props }) {
  const base = {
    display: 'inline-flex',
    alignItems: 'center',
  ...
  return (
    <button {...props} disabled={props.disabled || loading} style={{ ...base, ...variants[variant] }}>
      {loading ? (
        <>
          <span style={{
            width: 16, height: 16,
